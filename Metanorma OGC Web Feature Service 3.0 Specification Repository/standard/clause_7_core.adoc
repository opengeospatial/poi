...
A server that implements the POI API provides access to the features in a dataset.
In other words, the API is a distribution of that dataset. A file download, for example,
would be another distribution.

More specifically, each POI has a single `LandingPage` (path `/`) that provides links to
...
requirements classes implemented by the POI server.
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/30[ISSUE 30] +
Allow also features that do not belong to any collection?

CAUTION: link:https://github.com/opengeospatial/POI/issues/66[ISSUE 66] +
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/90[ISSUE 90] +
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/101[ISSUE 101] +
Landing page: Can we reuse existing relation types instead of 'conformance' and 'data'?

.link:https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/root.yaml[Schema for the landing page]
[source,YAML]
----
type: object
required:
...
properties:
  links:
    type: array
    items:
      $ref: https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/link.yaml
----
...
----
{
  "links": [
    ...
    { "href": "http://data.example.org/conformance",
      "rel": "conformance", "type": "application/json", "title": "POI conformance classes implemented by this server" },
...
Every POI provides an API definition that describes the capabilities
...
The idea is that any POI can be used by developers that are familiar with
...
.link:https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/req-classes.yaml[Schema for the list of requirements classes]
[source,YAML]
----
type: object
required:
...
{
  "conformsTo": [
    "http://www.opengis.net/spec/POI-1/3.0/req/core",
    "http://www.opengis.net/spec/POI-1/3.0/req/oas30",
    "http://www.opengis.net/spec/POI-1/3.0/req/html",
    "http://www.opengis.net/spec/POI-1/3.0/req/geojson"
  ]
}
----
=================
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/115[ISSUE 115] +
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/38[ISSUE 38] +
...
While the POI standard does not specify any mandatory encoding, we recommend
...
.link:https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/content.yaml[Schema for the metadata about feature collections]
[source,YAML]
----
type: object
required:
...
properties:
  ...
  collections:
    type: array
    items:
      $ref: https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/collectionInfo.yaml
----
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/56[ISSUE 56] +
Lack of DescribeFeatureType request

CAUTION: link:https://github.com/opengeospatial/POI/issues/102[ISSUE 102] +
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/103[ISSUE 103] +
...
.link:https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/collectionInfo.yaml[Schema for the metadata about a feature collection]
[source,YAML]
----
type: object
required:
...
properties:
  ...
  links:
    ...
    items:
      $ref: https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/link.yaml
  extent:
    $ref: https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/schemas/extent.yaml
  crs:
...
link:https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/parameters/limit.yaml[limit.yaml].
...
requirements will be specified in an additional part of the POI series.
...
A template for the definition of the parameter in YAML according to
OpenAPI 3.0 is available at
link:https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/parameters/bbox.yaml[bbox.yaml].
...
A template for the definition of the parameter in YAML according to
OpenAPI 3.0 is available at
link:https://raw.githubusercontent.com/opengeospatial/POI/master/core/openapi/parameters/time.yaml[time.yaml].
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/20[ISSUE 20] +
...
like the `startIndex` parameter that was used in previous versions of POI (and
...
CAUTION: Related to link:https://github.com/opengeospatial/POI/issues/8[ISSUE 8] +
...
CAUTION: link:https://github.com/opengeospatial/POI/issues/47[ISSUE 47] +
